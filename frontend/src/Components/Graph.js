import {Hint, LineMarkSeries, XYPlot, XAxis, YAxis, HorizontalGridLines, VerticalGridLines, LineSeries, Crosshair, DiscreteColorLegend} from 'react-vis';
...
import  {getWeeklyData, getMonthlyData, getAverageWeeklyData, getAverageMonthlyData} from '../formatData';
...
export default class Graph extends React.Component {
    ...
    _toggleButton = button => {
        if (button === 'weekly') {
          ...
          this._averageWeeklyData();
        }
        else if (button === 'averageMonthly') {
            this.setState({
                weekly: false,
                monthly: false,
                averageSpendings: false,
                averageSpendingsMonthly: true,
                nothing: true
          });
          this._averageMonthlyData();
        }
    ...
    _averageWeeklyData = () => {
        ...
        .then(res => {
            const data = getWeeklyData(res.data);
            const averageData = getAverageWeeklyData(data);
            this.setState({ data });
    ...
    _averageMonthlyData = () => {
        ...
        .then(res => {
            const data = getMonthlyData(res.data);
            const averageMonthlyData = getAverageMonthlyData(data);
            this.setState({ data });
    ...
    render() {
        if (this.state.nothing)
        {
            return (
                ...
                <Button id="btn-weekly" onClick={() => this._toggleButton('weekly')} active className="col-md-8">Current Week</Button>
                        ...
                        <Button id="btn-averageSpendingsMonthly" onClick={() => this._toggleButton('averageMonthly')} className="col-md-8 mt-3">Average Monthly Spendings</Button>                </div>
            </div>
            );
        }
            ...
            return (
                <div className="row">
                    <div className="col-md-3 text-center">
                        ...
                        <Button id="btn-monthly" onClick={() => this._toggleButton('monthly')} className="col-md-8 mt-3">Current Month</Button>
                        <Button id="btn-averageSpendings" onClick={() => this._toggleButton('average')} className="col-md-8 mt-3">Average Weekly Spendings</Button>
                        <Button id="btn-averageSpendingsMonthly" onClick={() => this._toggleButton('averageMonthly')} className="col-md-8 mt-3">Average Monthly Spendings</Button>
                    </div>
                ...
                <Spring from={{opacity: 0}} to={{opacity: 1}} className="col-md-8">
                        {props => (
                            <div className="col-md-9" style={props}>       
                                <XYPlot xType='ordinal' onMouseLeave={this._onMouseLeave} width={550} height={550} style={{backgroundColor: "#ffffff"}}>
                                <DiscreteColorLegend
                                    style={{position: 'absolute', left: '430px', top: '5px'}}
                                ...
                                />
                                    ...
                                    <LineMarkSeries onValueMouseOver={this._rememberValue} onValueMouseOut={this._forgetValue} onNearestX={this._onNearestX} data={this.state.data} />
                                    {this.state.value ? <Hint value={this.state.value}  /> : null}
                                    <LineMarkSeries color="orange" onValueMouseOver={this._rememberValue} onValueMouseOut={this._forgetValue} onNearestX={this._onNearestX} data={this.state.averageData} />
                                    {this.state.value ? <Hint value={this.state.value}  /> : null}
                                </XYPlot>
                            </div>
                        )}
        ...
        else if (this.state.loaded && this.state.averageSpendingsMonthly){
            return (
                <div className="row">
                <div className="col-md-3 text-center">
                    <Button id="btn-weekly" onClick={() => this._toggleButton('weekly')} className="col-md-8">Current Week</Button>
                    <Button id="btn-monthly" onClick={() => this._toggleButton('monthly')} className="col-md-8 mt-3">Current Month</Button>
                    <Button id="btn-averageSpendings" onClick={() => this._toggleButton('average')}  className="col-md-8 mt-3">Average Weekly Spendings</Button>
                    <Button id="btn-averageSpendingsMonthly" onClick={() => this._toggleButton('averageMonthly')} active className="col-md-8 mt-3">Average Monthly Spendings</Button>
                </div>
                <Spring from={{opacity: 0}} to={{opacity: 1}} className="col-md-8">
                        {props => (
                            <div className="col-md-9" style={props}>       
                                ...
                                />
                                    ...
                                    <LineMarkSeries onValueMouseOver={this._rememberValue} onValueMouseOut={this._forgetValue} onNearestX={this._onNearestX} data={this.state.data} />
                                    {this.state.value ? <Hint value={this.state.value}  /> : null}
                                    <LineMarkSeries color="orange" onValueMouseOver={this._rememberValue} onValueMouseOut={this._forgetValue} onNearestX={this._onNearestX} data={this.state.averageMonthlyData} />
                                    {this.state.value ? <Hint value={this.state.value}  /> : null}
                                </XYPlot>
                            </div>
                        )}
